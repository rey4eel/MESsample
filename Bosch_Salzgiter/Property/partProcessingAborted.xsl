  ...
  <xsl:template match="/">
	<xsl:element name="root"> <!-- root - start -->
		<xsl:element name="header"> <!-- header - start -->
			...
			<xsl:attribute name="eventName">partProcessingAborted</xsl:attribute>
		...
		<xsl:element name="event"> <!-- event - start -->
			<xsl:element name="partProcessingAborted"> <!-- partProcessingStarted - start -->
				<xsl:attribute name="identifier"><xsl:value-of select="$Barcode"/></xsl:attribute>
			</xsl:element> <!-- partProcessingStarted - end -->
		</xsl:element> <!-- event - end -->	
		<xsl:element name="body"> <!-- body - start -->
			<xsl:element name="structs">
				<xsl:element name="resHead">
		      			<xsl:attribute name="result"><xsl:text>3</xsl:text></xsl:attribute>
					<xsl:choose>
			             ...
			             <xsl:attribute name="workingCode"><xsl:value-of select="$workingCode"/></xsl:attribute>
		      			<xsl:attribute name="nioBits"><xsl:text>0</xsl:text></xsl:attribute>
