  ...
  <xsl:template match="/">
	<xsl:element name="root"> <!-- root - start -->
		<xsl:element name="header"> <!-- header - start -->
			...
			<xsl:attribute name="eventName">plcOperationModeChanged</xsl:attribute>
		...
		<xsl:element name="event"> <!-- event - start -->
			<xsl:element name="plcOperationModeChanged"> <!-- plcOperationModeChanged start -->
				<xsl:choose>
		                <xsl:when test="$Lane='0'"> <!-- Front -->
				...
				<xsl:attribute name="shift"><xsl:value-of select="$shift"/></xsl:attribute>
				<xsl:attribute name="charge"><xsl:value-of select="$charge"/></xsl:attribute>
				<xsl:attribute name="specPrgNo"><xsl:value-of select="$specPrgNo"/></xsl:attribute>
			...
			</xsl:element> <!-- plcOperationModeChanged end -->
