...
<xsl:param name="COFrontTypeNo"/>
<xsl:param name="COFrontTypeVar"/>
<xsl:param name="CORearTypeNo"/>
<xsl:param name="CORearTypeVar"/>
  ...
  <xsl:template match="/">
	<xsl:element name="root"> <!-- root - start -->
		<xsl:element name="header"> <!-- header - start -->
			...
			<xsl:attribute name="eventName">dataDownloadRequired</xsl:attribute>
		...
		<xsl:element name="event"> <!-- event - start -->
			<xsl:element name="dataDownloadRequired"> <!-- dataDownloadRequired - start -->
			</xsl:element> <!-- dataDownloadRequired - end -->
		</xsl:element> <!-- event - end -->	
		
		
		<xsl:element name="body"> <!-- body - start -->
			<xsl:element name="items"> <!-- item - start -->
			
				<xsl:element name="item"> <!-- dataDownloadRequired - start -->
		                	<xsl:attribute name="name"><xsl:text>typeNo</xsl:text></xsl:attribute>
					<xsl:choose>
			                <xsl:when test="$Lane='0'"> <!-- Front -->
						<xsl:attribute name="value"><xsl:value-of select="$COFrontTypeNo"/></xsl:attribute>
			                </xsl:when>
			                <xsl:otherwise>   <!-- Rear -->
						<xsl:attribute name="value"><xsl:value-of select="$CORearTypeNo"/></xsl:attribute>
			                </xsl:otherwise>		
					</xsl:choose>
					<xsl:attribute name="dataType"><xsl:text>8</xsl:text></xsl:attribute>
				</xsl:element> <!-- partReceived - end -->
				<xsl:element name="item"> <!-- dataDownloadRequired - start -->
		                	<xsl:attribute name="name"><xsl:text>typeVar</xsl:text></xsl:attribute>
					<xsl:choose>
			                <xsl:when test="$Lane='0'"> <!-- Front -->
						<xsl:attribute name="value"><xsl:value-of select="$COFrontTypeVar"/></xsl:attribute>
			                </xsl:when>
			                <xsl:otherwise>   <!-- Rear -->
						<xsl:attribute name="value"><xsl:value-of select="$CORearTypeVar"/></xsl:attribute>
			                </xsl:otherwise>		
					</xsl:choose>
					<xsl:attribute name="dataType"><xsl:text>8</xsl:text></xsl:attribute>
				</xsl:element> <!-- partReceived - end -->
