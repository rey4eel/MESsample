  ...
  <xsl:template match="/">
	<xsl:element name="root"> <!-- root - start -->
		<xsl:element name="header"> <!-- header - start -->
			...
			<xsl:attribute name="eventName">plcError</xsl:attribute>
		...
		<xsl:element name="event"> <!-- event - start -->
			<xsl:element name="plcError"> <!-- plcError start -->
				<xsl:choose>
		                <xsl:when test="$Lane='0'"> <!-- Front -->
				...
				<xsl:attribute name="errorNo"><xsl:value-of select="$errorNo"/></xsl:attribute>
				<xsl:attribute name="errorText"><xsl:value-of select="$errorText"/></xsl:attribute>
				<xsl:attribute name="errorType"><xsl:value-of select="$errorType"/></xsl:attribute>
				<xsl:attribute name="errorState"><xsl:value-of select="$errorState"/></xsl:attribute>
				<xsl:attribute name="operationMode"><xsl:value-of select="$operationMode"/></xsl:attribute>
				<xsl:attribute name="modeOn"><xsl:value-of select="cs:BoolToString($modeOn)"/></xsl:attribute>
				<xsl:attribute name="chainNo"><xsl:value-of select="$chainNo"/></xsl:attribute>
				
			</xsl:element> <!-- plcError end -->
