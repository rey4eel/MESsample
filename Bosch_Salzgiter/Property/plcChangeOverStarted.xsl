  ...
  <xsl:template match="/">
	<xsl:element name="root"> <!-- root - start -->
		<xsl:element name="header"> <!-- header - start -->
			...
			<xsl:attribute name="eventName">plcChangeOverStarted</xsl:attribute>
		...
		<xsl:element name="event"> <!-- event - start -->
			<xsl:element name="plcChangeOverStarted"> <!-- plcChangeOverStarted - start -->
				<xsl:choose>
		                <xsl:when test="$Lane='0'"> <!-- Front -->
					<xsl:attribute name="typeNo"><xsl:value-of select="$COFrontTypeNo"/></xsl:attribute>
					<xsl:attribute name="typeVar"><xsl:value-of select="$COFrontTypeVar"/></xsl:attribute>
		                </xsl:when>
		                <xsl:otherwise>   <!-- Rear -->
					<xsl:attribute name="typeNo"><xsl:value-of select="$CORearTypeNo"/></xsl:attribute>
					<xsl:attribute name="typeVar"><xsl:value-of select="$CORearTypeVar"/></xsl:attribute>
		                </xsl:otherwise>		
				</xsl:choose>
			</xsl:element> <!-- plcChangeOverStarted - end -->
